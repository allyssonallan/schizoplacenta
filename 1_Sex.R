...
# Capture the ComplexHeatmap output as a grid object
heatmap_grob <- grid.grabExpr(draw(heatmap))

# Convert the grid object to a ggplot object
heatmap_ggplot <- as.ggplot(heatmap_grob)

# Display the ggplotified heatmap
...
combined <- combined + plot_layout(ncol = 1, heights = c(0.5, 1))
ggsave("results_paper/4_Sex.png", combined, width = 300, height = 300, units = "mm", dpi = 400)
