...
log2.cpm.filtered.norm.df.pivot <- pivot_longer(log2.cpm.filtered.norm.df,
                                                cols = AAL_fetal_S8:SLS_fetal_S11,
                                                names_to = "samples",
...
healthy_concatenated <- paste(healthy_concatenated$masctranscRN1, collapse = " + ")
altered_concatenated <- altered[!duplicated(altered$masctranscRN1),]
altered_concatenated <- paste(altered_concatenated$masctranscRN1, collapse = " + ")
...
write.table(diffGenes.df, file = "All_Fetal_Mot_DEG.tsv", sep = "\t", row.names = FALSE, col.names = TRUE)
...
write.table(top50_genes, file = "top50Motor.tsv", sep = "\t", row.names = FALSE, col.names = TRUE)

df_clean <- na.omit(top50_genes)
df_clean <- df_clean[c("geneID", "logFC", "P.Value", "adj.P.Val")]
#df_clean <- df_clean[df_clean$adj.P.Val <= 0.75,]
