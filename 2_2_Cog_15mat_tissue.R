...
groupcont <- factor(targetsnd$masctransM)
...
#### Create PCA plots ####
p1_m <- ggplot(pca.res.df, aes(x = PC1, y = PC2, color = groupcont)) +
  geom_point(size = 4) +
  stat_ellipse() +
...
write.table(top50_genes, file = "top50Cog15Mat.tsv", sep = "\t", row.names = FALSE, col.names = TRUE)
write.table(myTopHits.df, file = "CogAllMat.tsv", sep = "\t", row.names = FALSE, col.names = TRUE)
...
combined = combined +
  plot_annotation(title = "Cognitive DEGs category",
                  tag_levels = 'a')
combined
ggsave("results_paper/Fig_Cog_Tissue.png", combined, width = 600, height = 400, units = "mm", dpi = 400)
ggsave("results_paper/Fig_Cog_Tissue.tiff", combined, width = 600, height = 400, units = "mm", dpi = 400)
...
write.table(diffGenes.df, file = "3_1_Cog_DEG_mat_tissue.tsv", sep = "\t", row.names = FALSE, col.names = TRUE)
